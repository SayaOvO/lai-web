...
import { ComponentVNode, h, hFragment, hString } from '../h'
...
describe('Component props', () => {
  ...
  test("can't patch the DOM if the component isn't mounted", () => {
    const comp = new PropsComp({ pClass: 'definition' })
    expect(() => comp.updateProps({ pClass: 'lemma' })).toThrow(/not mounted/)
  })
...
describe('Component state', () => {
  ...
  test('can be based on the props', () => {
    const Comp = defineComponent<State, Props>({
      state(props) {
        return { count: props ? props.initialCount : 0 }
      },
      render() {
...
describe('Patching the DOM', () => {
  test('can add event handlers bound to the component', async () => {
    ...
    const Component = defineComponent<{ count: number }>({
      state() {
        return { count: 0 }
      },
      render() {
    ...
    ;(document.querySelector('#plus-btn') as HTMLButtonElement).click()
    await nextTick()
    expect(document.body.innerHTML).toBe(
      '<button id="minus-btn">-</button><span>1</span><button id="plus-btn">+</button>'
    )
    ;(document.querySelector('#minus-btn') as HTMLButtonElement).click()
    expect(document.body.innerHTML).toBe(
      '<span>0</span><button id="plus-btn">+</button>'
    )
  })
...
describe('Offset', () => {
  test('when the components top-level element is a fragment, it has an offset', () => {
    ...
    const firstComponent = (vdom.children[0] as ComponentVNode).component
    const secondComponent = (vdom.children[1] as ComponentVNode).component
    const thirdComponent = (vdom.children[2] as ComponentVNode).component

    if (firstComponent && secondComponent && thirdComponent) {
  ...
  test('when the components top-level element is not a fragment, it has NO offset', () => {
    ...
    const secondComponent = (vdom.children[1] as ComponentVNode).component
    const thirdComponent = (vdom.children[2] as ComponentVNode).component
    console.log('first:', firstComponent?.firstElement)
    if (firstComponent && secondComponent && thirdComponent) {
      expect(firstComponent.offset).toBe(0)
...
const PropsComp = defineComponent<{}, { pClass: string | string[] }>({
  render() {
...
const StateComp = defineComponent<State, Props>({
  state() {
    return { count: 0 }
  },
  render() {
...
const ListItem = defineComponent<{ highlighted: boolean }, { text: string }>({
  state() {
